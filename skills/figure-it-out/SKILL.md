...
- The definition of done as a falsifiable predicate, per the **prove-it-works** principle skill.
...
- For one-way-door design decisions, run the **architect** skill, which runs **arena**. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill).
...
Check the whole against the Phase A predicate on the real product, not just the harness. Encode any recurring correction as a gate, a lint rule, a check, or a script, per the **encode-lessons-in-structure** principle skill.
