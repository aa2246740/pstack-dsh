...
Look recursively for `.dsh/skills/**/*-mode/SKILL.md`, `$DSH_HOME/skills/*-mode/SKILL.md`, and `.agents/skills/**/*-mode/SKILL.md` matching the user's handle. If one exists, confirm intent with `ask_user_question` (unless they already said "update my skill" or similar):

- Update the existing skill (default for repeat runs)
- Start fresh. Ask why before doing it.
...
Locate the active workspace's transcripts before fanning out. Use this DSH session log and session-query tools when present. Don't glob other users' `$DSH_HOME`.

Survey recent agent conversations within that scope for recurring patterns. Run multiple parallel subagents across slices of history (e.g. last 2-4 weeks, split into 3 slices so each has enough material). Each slice mining subagent reads transcripts from the workspace-scoped path the parent provides, looks for the signals below, and returns a short structured list of patterns it saw with evidence pointers. Default signals worth hunting:

- Response preferences (length, tone, format, "dumb it down" corrections)
- Delegation habits (subagents, models, specialized workflows, parallelism)
- Verification posture, including what "done" means, unit tests vs live repro, and reviewers
...
- Autonomy, including how much to do without asking and MCP tool use.
...
Use the **authoring-a-skill** playbook skill to author the skill. Placement:

- Path: preserve an existing mode skill's category. For a new mode, use `.dsh/skills/<handle>-mode/SKILL.md` in the project (or `$DSH_HOME/skills/<handle>-mode/` if the user prefers a personal skill).
- Handle: the user's first name or chosen identifier.
- Frontmatter `description`: trigger on their name + `/<handle>-mode` + "work in their style", not on generic keywords like "write code" or "review PR".
- Frontmatter formatting: kebab-case `name`, required `description`. Keep `description` as one YAML scalar. Quote it or use `description: >-` with indented continuation lines when punctuation or wrapping requires it.
...
Work in a worktree off the repository's default branch. Commit and open a review request with its forge tools. Don't push to the default branch directly.
...
A `-mode` skill is subjective output. A benchmark loop isn't useful here. Vibe-check with the user: does it read like them? Did it miss anything? Then ship.
...
- User wants to capture one narrow workflow, such as "how I write commit messages". That's a regular skill, not a mode skill.
