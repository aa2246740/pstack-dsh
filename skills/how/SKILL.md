...
Once all explorers return, spawn a single `pstack_spawn` (`role: how-explainer`) to synthesize their findings into one coherent explanation. Do not send `model`. Overlay or inherit. Instruct no writes in the prompt.
...
After the explanation is complete, spawn critics from overlay role `how-critics` (see spawn.md). If the overlay inherits, spawn two independent critics both inheriting this conversation. If it lists routes, one `pstack_spawn` per route with `route_index`. Do not invent slugs to fake a panel.

For each critic: `pstack_spawn` `role: how-critics`, `run_in_background: true`. Instruct no writes in the prompt.
