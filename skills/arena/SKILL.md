...
After all Phase B candidates complete, pick a judge from overlay `arena-cross-judge-pool` (a route whose family differs from the parent when the catalog has one). If the pool inherits, the judge inherits this conversation. Spawn one `pstack_spawn` `role: arena-cross-judge-pool`. Instruct no writes. It sees the rubric and the candidates by path label, scores each criterion, and recommends a base with rationale. It runs in parallel with the parent's reading in Phase D, not with the candidates themselves. Spawning while candidates are still writing means the judge sees partial or empty outputs and reports them as dropouts.
