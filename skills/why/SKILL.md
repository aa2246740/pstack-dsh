...
Subagent config (each): `pstack_spawn` `role: why-investigators`, `run_in_background: true`. Do not send `model`. Overlay or inherit. **Do not send `readonly`.** DSH `subagent` has no such field, and a fake readonly flag would not strip tools. Investigators still shouldn't write anything. That's a posture, not a sandbox. Follow [`../setup-pstack/references/spawn.md`](../setup-pstack/references/spawn.md).
