...
Distinct from **Runtime forensics**, which instruments the live process. Here the capture already exists. Read the fixed dataset, don't re-run it. Keep tooling generic: a DevTools or trace parser for cpuprofile and `.json.gz`, a text editor for a spindump, your heap tooling for a heapsnapshot.

1. Identify the format and load it with the right tool. Parse large artifacts in a subagent per **principle-guard-the-context-window** and keep the reduced finding in the main thread.
