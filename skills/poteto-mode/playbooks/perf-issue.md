...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation with `pstack_spawn` `role: perf-issue` (overlay or inherit); review the diff. Capture a post-fix trace.
