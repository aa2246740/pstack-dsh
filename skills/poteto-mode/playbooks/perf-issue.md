...
2. `how` to ground hypotheses. Don't claim a perf ceiling without running it first.
   Use these eight strategy families as hypothesis generators, not a checklist. A family earns an attempt only when the trace shows the signal it names.
   ...
   - **Batching.** Many small operations each pay a fixed overhead, such as an RPC, query, syscall, or draw call. Coalesce them to pay the overhead once per batch.
   - **Redundancy.** The wait hangs on one slow instance or attempt. Duplicate the work through replicas, hedged requests, or speculative execution and take the fastest result. The trace has to show the wait dominates and the system has headroom.
   - **Lazy evaluation.** Cost lands on results never used or not needed yet, such as eager init on the boot path or rendering offscreen items. Defer the work until first use.
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation with `pstack_spawn` `role: perf-issue`. Routing uses the overlay or inherits the parent. Review the diff. Capture a post-fix trace.
   Apply the **sequence-verifiable-units** principle skill, verifying each attempt before trying the next.
4. Parse and compare the artifacts, such as JSON to sqlite and a diff. "Inconclusive" or wrong-surface is not a pass. Flag it.
5. Cite the measurement in the PR.
6. Run **Opening a PR**.

For sustained improvement against a metric rather than a one-off fix, use `playbooks/hillclimb.md`.
