...
- Prefer evidence produced by committed scripts over hand-made one-offs, per the **encode-lessons-in-structure** principle skill.

## Audit the log against the transcript

At the end of the run, before handing back, check the log told the truth. Read this run's DSH session log through the current session or available session-query tools. If no transcript is available, report that audit limit. Don't glob other users' `$DSH_HOME` or Cursor project dirs. Walk the log against what actually happened:
...
Before handing back, use `pstack_spawn` with `role: poteto-agent` for an independent reviewer. Use `pstack_overlay_read` to select a mapped route from a different model family when available. Otherwise inherit and report the diversity limit. Self-review is not a substitute. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
...
Read top to bottom, follow the evidence pointers, spot-check. Use the forge's file viewer or `column -s$'\t' -t decisions.tsv` in a terminal.
