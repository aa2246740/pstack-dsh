...
1. Spawn `pstack_spawn` with `role: "comment-sicko"`. Pass the scope. Do not restate its rules. Follow [`../setup-pstack/references/spawn.md`](../setup-pstack/references/spawn.md).
...
4. Implement the smallest root-cause fix in scope. Remove every named workaround. If the root cause is out of scope, land the smallest in-scope fix and report the rest open. The **principle-fix-root-causes** and **principle-redesign-from-first-principles** skills guide intent only. Neither authorizes widening the scope nor fixing instances outside it. Never bolt on symptom guards.
